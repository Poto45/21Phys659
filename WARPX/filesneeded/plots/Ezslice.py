...
os.mkdir(pythonpath + '/Ezsliceat0_' + tstamp)
...
for it in  steps:
        ...
        plt.figure()
        plt.plot(np.arange(0,shE[0]),Ez[:,int(shE[1]/2),int(shE[2]/2)])#,cmap='RdBu',vmin=-1000,vmax=1000)#extent=[info_Ez.zmin*1e3,info_Ez.zmax*1e3,info_Ez.ymin*1e3,info_Ez.ymax*1e3],cmap='RdBu',vmin=-absmax,vmax=absmax,aspect='auto')
        plt.ylabel('$E_z$')
        plt.xlabel('$z$ (mm)')
#        plt.title('Ez')
        plt.savefig(path2hdf5 + 'Ez_sliceatzero'+str(it))
        shutil.move(path2hdf5 + 'Ez_sliceatzero'+str(it)+'.png',pythonpath + '/Ezsliceat0_' + tstamp) 
